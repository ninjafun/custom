...
namespace NinjaTrader.Indicator
{
    ...
    /// </summary>
    [Description("EMAs")]
    public class GrabShort : Indicator
    {
        #region Variables
        // Wizard generated variables
        private int period = 34; // Default setting for Period
        private Color barColorDown = Color.Red;//.Red;
        private Color barColorUp = Color.Maroon;//.Lime;
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private GrabShort[] cacheGrabShort = null;

        private static GrabShort checkGrabShort = new GrabShort();

        /// <summary>
        /// EMAs
        /// </summary>
        /// <returns></returns>
        public GrabShort GrabShort(int period)
        {
            return GrabShort(Input, period);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public GrabShort GrabShort(Data.IDataSeries input, int period)
        {
            if (cacheGrabShort != null)
                for (int idx = 0; idx < cacheGrabShort.Length; idx++)
                    if (cacheGrabShort[idx].Period == period && cacheGrabShort[idx].EqualsInput(input))
                        return cacheGrabShort[idx];

            lock (checkGrabShort)
            {
                checkGrabShort.Period = period;
                period = checkGrabShort.Period;
                ...
                GrabShort indicator = new GrabShort();
                ...
                GrabShort[] tmp = new GrabShort[cacheGrabShort == null ? 1 : cacheGrabShort.Length + 1];
                if (cacheGrabShort != null)
                    cacheGrabShort.CopyTo(tmp, 0);
...
namespace NinjaTrader.MarketAnalyzer
{
    public partial class Column : ColumnBase
    {
        ...
        public Indicator.GrabShort GrabShort(int period)
        {
            return _indicator.GrabShort(Input, period);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Indicator.GrabShort GrabShort(Data.IDataSeries input, int period)
        {
            return _indicator.GrabShort(input, period);
        }
    }
