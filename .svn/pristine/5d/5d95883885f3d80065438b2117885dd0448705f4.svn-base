...
namespace NinjaTrader.Indicator
{
    ...
    /// </summary>
    [Description("EMAs")]
    public class GrabLong : Indicator
    {
        #region Variables
        // Wizard generated variables
            private int period = 34; // Default setting for Period
            private Color barColorDown = Color.Green;//.Red;
            private Color barColorUp = Color.Chartreuse;//.Lime;
...
namespace NinjaTrader.Indicator
{
    ...
    {
        private GrabLong[] cacheGrabLong = null;

        private static GrabLong checkGrabLong = new GrabLong();

        /// <summary>
        /// EMAs
        /// </summary>
        /// <returns></returns>
        public GrabLong GrabLong(int period)
        {
            return GrabLong(Input, period);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public GrabLong GrabLong(Data.IDataSeries input, int period)
        {
            if (cacheGrabLong != null)
                for (int idx = 0; idx < cacheGrabLong.Length; idx++)
                    if (cacheGrabLong[idx].Period == period && cacheGrabLong[idx].EqualsInput(input))
                        return cacheGrabLong[idx];

            lock (checkGrabLong)
            {
                checkGrabLong.Period = period;
                period = checkGrabLong.Period;
                ...
                GrabLong indicator = new GrabLong();
                ...
                GrabLong[] tmp = new GrabLong[cacheGrabLong == null ? 1 : cacheGrabLong.Length + 1];
                if (cacheGrabLong != null)
                    cacheGrabLong.CopyTo(tmp, 0);
...
namespace NinjaTrader.MarketAnalyzer
{
    public partial class Column : ColumnBase
    {
        ...
        public Indicator.GrabLong GrabLong(int period)
        {
            return _indicator.GrabLong(Input, period);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Indicator.GrabLong GrabLong(Data.IDataSeries input, int period)
        {
            return _indicator.GrabLong(input, period);
        }
    }
